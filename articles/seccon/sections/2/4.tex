...
となると、\Marsはまず間違いなくOSコマンドインジェクションか、メールヘッダ汚染。
...
つまり、\ctt{mail\_to}クエリに何か、例えば次のようなものを差し込む。
...
この例では\ctt{sendmail}と意図しない\ctt{ls}が同時に実行されることになる。
...
現実の世界であれば、\ctt{message.cgi}ようなメールを送信するプログラムは、
確実にインターネットへ接続されている。
しかし\Marsは競技という性質上、
