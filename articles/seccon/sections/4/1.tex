...
よし。
昨日おーぴーに言われて用意した\ctt{pkill.php}というゴミアプリがある。
これは、
...
これではっきりした。
たぶん俺のメールヘッダ汚染は失敗だ。
あの\Jupiterは完全にVMの上で置いてあるので、
恐らく\Marsなど他の問題もVMに設置されているだろう。
だから多分俺のコンピュータに設置されたサーバーへアクセス出来ない。

青い溜息を吐いて、解ける見込の失せた\Marsを見る。
...
あるいは何か別の理由なのか。
いずれにしても\mofuppの命運が尽きたことに少々安堵した。
